...
export default class CarritoCompra {
  constructor({ idCliente }) {
    ...
    this.enviosDraft = [];    // [{ idTemporal, dto, precio }]
  ...
  calcularTotal() {
    const totalTiq = this.tiquetesDraft.reduce(
      (acc, t) => acc + (t.precio || 0),
      0
    );
    const totalEnv = this.enviosDraft.reduce(
  ...
  crearMemento() {
    const state = {
      ...
      // copiamos estructuras para no compartir referencias
      tiquetesDraft: JSON.parse(JSON.stringify(this.tiquetesDraft)),
      enviosDraft: JSON.parse(JSON.stringify(this.enviosDraft)),
  ...
  restaurarDesdeMemento(memento) {
    ...
    this.descuentoGlobal = state.descuentoGlobal || 0;
  }
}
